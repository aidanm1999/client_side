...
function gettalks()
{
    ...
    $.ajax({url: "http://localhost:3000/sessions", success: function(sessions){
        $.ajax({url: "http://localhost:3000/talks", success: function(talks){
            globalTalks = talks;
            ...
            for (i = 0; i < talks.length; i++) {
                ...
                yourRatingText.addEventListener("change", (function (event){
                    $.ajax({url: "http://localhost:3000/talks/rate/"+event.target.id+"/"+ event.target.value, success: function(result){
                        var newAvg = 0;
